...
def build_similarity_counts(texts: list[str], threshold: float) -> list[int]:
    clusters = cluster_by_similarity(texts, threshold=threshold)
    counts = [1] * len(texts)
...
def process_tickets(frame: pd.DataFrame, config: ProcessConfig) -> pd.DataFrame:
    ...
    similarity_counts = build_similarity_counts(
        combined_texts, threshold=config.similarity_threshold
    )
